...
export async function snapshotBossPageViewport(page: Page): Promise<BossViewportSnapshot> {
  ...
  const dims = await page.evaluate("(() => ({ width: window.innerWidth, height: window.innerHeight, dpr: window.devicePixelRatio || 1 }))()") as { width: number; height: number; dpr: number };
